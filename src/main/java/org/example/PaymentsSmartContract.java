...
@Contract(name = "PaymentsSmartContractC",
...
@Default
public class PaymentsSmartContract implements ContractInterface {
    ...
    @Transaction()
    public Payments readPaymentsSmartContract(Context ctx, String paymentsSmartContractId) {
        boolean exists = paymentsSmartContractExists(ctx,paymentsSmartContractId);
        if (!exists) {
            throw new RuntimeException("The asset "+paymentsSmartContractId+" does not exist");
        }

        Payments newAsset = Payments.fromJSONString(new String(ctx.getStub().getState(paymentsSmartContractId),UTF_8));
        return newAsset;
    }
